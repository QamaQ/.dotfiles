return {
    "neovim/nvim-lspconfig",                -- Plugin para configurar servidores de lenguaje (LSP)
    event = { "BufReadPre", "BufNewFile" }, -- Carga el plugin al abrir o crear un nuevo buffer
    -- enabled = false,                             -- Asegura que el plugin esté habilitado
    dependencies = {
        {
            "folke/lazydev.nvim",
            ft = "lua",
    ...
    config = function()                          -- Función de configuración que se ejecuta cuando el plugin es cargado
        ...
        -- Configuración global de diagnósticos (errores, advertencias, etc.)
        ...
        -- Configuración de servidores de lenguaje específicos
        -- Aquí se configuran 'lua_ls' (para Lua/Neovim) y 'pyright' (para Python).

        -- Servidor de lenguaje para Lua (neovim)
        vim.lsp.config("lua_ls", {
            on_attach = on_attach,       -- Asigna la función `on_attach` definida anteriormente
            capabilities = capabilities, -- Pasa las capacidades del cliente
            -- settings = {              -- Configuraciones específicas del servidor (comentadas)
            --     Lua = {
        ...
        -- Servidor de lenguaje para Python
        vim.lsp.config("pyright", {
            on_attach = on_attach,       -- Asigna la función `on_attach`
            capabilities = capabilities, -- Pasa las capacidades del cliente
        })

        -- Habilita los servidores de lenguaje configurados
        vim.lsp.enable({ "lua_ls", "pyright", "python", "qmlls" }) -- Inicia los servidores LSP especificados
    end,
}
